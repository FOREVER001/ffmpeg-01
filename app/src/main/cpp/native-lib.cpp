...
Java_com_tianzhuan_wangyiplayer_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
...
Java_com_tianzhuan_wangyiplayer_WangyiPlayer_native_1start(JNIEnv *env, jobject instance,
...
//    ret为零 表示成功
    ...
    int find_stream_info=avformat_find_stream_info(formatContext, NULL);
...
//    ANativeWindow
    ...
    while (av_read_frame(formatContext, packet) == 0) {
        ...
        if (packet->stream_index == vidio_stream_idx) {// 取到一帧视频帧
//非零   正在解码
            if (ret==0) {
//            绘制之前   配置一些信息  比如宽高   格式
